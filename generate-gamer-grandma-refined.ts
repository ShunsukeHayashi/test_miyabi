...
/**
 * Gamer Grandma Video Generator - Refined Workflow
 *
 * Workflow:
 * 1. T2I: Generate base character image
 * 2. Review and select best character
 * 3. I2I: Create scene variations from base character
 * 4. I2V: Generate videos using scene images as reference
 *
...
/**
 * Let user select best character
 */
async function selectCharacter(characters: Array<{ id: number; url: string; name: string }>): Promise<string> {
  console.log('\n' + '='.repeat(60));
  ...
  console.log('Please open the URLs in your browser and review the characters.');
  console.log('Select the character that best fits the "gamer grandma" concept.\n');

  const answer = await question('Enter the character number (1-3) to use for all scenes: ');
  const selectedId = parseInt(answer.trim());

  if (selectedId < 1 || selectedId > 3) {
    console.log('❌ Invalid selection. Using Character 1 by default.');
  ...
  console.log(`\n✅ Selected: Character ${selected.id} - ${selected.name}`);
...
async function saveMetadata(data: {
  ...
  const metadata = {
    title: 'Gamer Grandma - Refined Workflow',
...
async function main() {
  ...
  console.log('  Phase 2: Create scene variations from selected character (I2I)');
  console.log('  Phase 3: Animate scenes into videos (I2V)');
  console.log('='.repeat(60));
  ...
  try {
    ...
    // User selection
    const selectedCharacter = await selectCharacter(characters);

    // Phase 2: Generate scene variations
    const scenes = await generateSceneVariations(selectedCharacter);
